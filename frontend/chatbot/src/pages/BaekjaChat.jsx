...
export default function BaekjaChat() {
  const [messages, setMessages] = useState(() => {
    const saved = localStorage.getItem('chat_messages_baekja');
  ...
  const audioRef = useRef(null); // 오디오 객체 저장용 ref

  // 사용자 ID 설정
  useEffect(() => {
    let storedId = localStorage.getItem('user_id_baekja');
    if (!storedId) {
      storedId = crypto.randomUUID();
      localStorage.setItem('user_id_baekja', storedId);
    }
  }, []);
  ...
  const handleSend = async () => {
    ...
    const userId = localStorage.getItem('user_id_baekja');
    const userMessage = { sender: 'user', text: input };

    setMessages((prev) => {
      const updated = [...prev, userMessage];
      localStorage.setItem('chat_messages_baekja', JSON.stringify(updated));
      return updated;
    });
    ...
    <Layout>
      <ChatWrapper>
        ...
        <TopBar>
          ...
          {menuOpen && (
            <DropdownMenu>
              <MenuItem onClick={() => navigate('/baekja/detail')}>
                <FiEye /> 유물 살펴보기
              </MenuItem>
              ...
              <MenuItem
                onClick={() => {
                  localStorage.removeItem('chat_messages_baekja');
                  setMessages([]);
                  setMenuOpen(false);
                }}
              >
